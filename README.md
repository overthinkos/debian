...
| `deploy:` | `debian-debootstrap-vm` (disposable bootstrap-VM bed) |
...
## debootstrap-from-scratch (`debian-debootstrap` / `debian-debootstrap-vm`)
...
builder:debootstrap`). `debian-debootstrap-vm` boots that rootfs under
libvirt/QEMU and carries `disposable: true`, so `ov -C image/debian update
debian-debootstrap-vm` rebuilds it unattended.
